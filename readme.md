...
* 优点
  * 视觉美感：配色清新；动态效果；定制皮肤；特别设计猫猫座位
  * 人性化设计：页面明了，指引清晰，易操作；互动体验感强
  * 内涵彩蛋，敬请期待~
...
- `seat.css`, `movie.css`, `payment.css`, `orders.css`: **视图模块样式**，每个文件对应一个具体功能视图的独立样式，实现了样式的模块化。
- `responsive.css`: 响应式样式，用于适配不同尺寸的设备（如手机、平板），确保在各种屏幕上都有良好的用户体验。

**js/ (脚本文件夹)**

此文件夹是项目的核心，存放所有的JavaScript逻辑代码，并遵循**数据与UI分离**的原则。

- `main.js`: **核心数据与算法模块**。负责管理所有核心数据结构（如座位、票务），并提供业务逻辑算法（如自动选座、票务处理）。此模块不直接与任何UI元素（DOM）交互。
- `stateManager.js`: **状态管理模块**。负责管理和同步整个应用的全局状态，如用户选择的座位、当前登录信息、选中的电影等，是各模块间通信的桥梁。
...
- `ui-validation.js`: **前端验证模块**。负责处理所有前端的业务逻辑验证，如选座规则是否满足、表单输入是否合法等。
- `canvas.js`: **座位图渲染模块**。专门负责在HTML Canvas上绘制和更新复杂的座位图，包括不同状态座位的渲染和交互事件的监听。

**img/ & pic/ (资源文件夹)**

为了清晰地区分资源用途，项目使用了两个图片文件夹：

- `img/`: 存放**UI界面直接使用**的图片资源，例如电影海报、应用背景图、以及表示不同状态的座位图标 (`seat_available.PNG`, `seat_sold.PNG` 等)。
- `pic/`: 存放用于**项目文档、演示和说明**的图片，例如项目Logo、功能流程图 (`purchase_flow_chart.jpg`)、以及不同界面的功能截图等。这些图片不被应用程序直接调用。
